...
{
    private const string MutexName = @"Local\ScreenshotTray.SingleInstance";
    private const string SurfaceSignalName = @"Local\ScreenshotTray.ShowWindow";
    private const string QuitSignalName = @"Local\ScreenshotTray.Quit";
    ...
    private static SingleInstance? TryTakeSlot(Action onSurfaceRequest, Action onQuitRequest)
    {
        ...
        {
            IsBackground = true,
            Name = "ScreenshotTray single-instance listener",
        };
    ...
    private static RunningBuild? FindOtherInstance()
    {
        ...
        foreach (var process in Process.GetProcessesByName("ScreenshotTray"))
        {
            using (process)
            {
                if (process.Id == self)
