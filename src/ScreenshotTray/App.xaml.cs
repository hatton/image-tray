...
public partial class App : Application
{
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        // A second launch of the same exe surfaces the instance already running
        // rather than adding a competing tray icon. A launch of a *different* build
        // is a different situation, and never a silent one.
        var launch = SingleInstance.Claim(
            () => Dispatcher.InvokeAsync(ShowWindow),
            () => Dispatcher.InvokeAsync(QuitApp));

        if (launch.Outcome == LaunchOutcome.SurfacedExisting)
        {
        ...
        if (launch.Outcome == LaunchOutcome.BlockedByOtherBuild)
        {
            ReportOtherBuild(launch.Other!);
            Shutdown();
            return;
        }

        _singleInstance = launch.Instance;
    ...
    /// <summary>
    /// Says which exe won the single-instance slot, because the alternative is a
    /// window that looks like this build's and is not.
    /// </summary>
    private static void ReportOtherBuild(RunningBuild other)
    {
        var mine = Environment.ProcessPath;
        var minePart = string.IsNullOrEmpty(mine)
            ? "This one could not work out its own path."
            : $"This one:\n{mine}\nbuilt {File.GetLastWriteTime(mine):g}";

        Log.Info($"Did not start: '{other.Path}' (built {other.BuiltAt:g}) already holds the slot.");

        MessageBox.Show(
            "A different build of Screenshot Tray is already running, so this one has not started.\n\n" +
            $"Already running:\n{other.Path}\nbuilt {other.BuiltAt:g}\n\n" +
            $"{minePart}\n\n" +
            "Quit the running one from its tray icon, then start this one again.",
            "Screenshot Tray",
            MessageBoxButton.OK,
    ...
    private bool ConfirmBulkRotation(string folder, int count)
    {
        ...
        var message =
            $"Screenshot Tray keeps the newest {keep} screenshots in:\n\n{folder}\n\n" +
        ...
        var answer = owner is null
            ? MessageBox.Show(message, "Screenshot Tray", MessageBoxButton.YesNo, MessageBoxImage.Question)
            : MessageBox.Show(owner, message, "Screenshot Tray", MessageBoxButton.YesNo, MessageBoxImage.Question);
    ...
    private void CreateTrayIcon()
    {
        ...
        menu.Items.Add(BuildMenuItem("Show Screenshot Tray", ShowWindow));
        ...
        menu.Items.Add(BuildMenuItem("Exit Screenshot Tray", QuitApp));

        _trayIcon = new TaskbarIcon
        {
            IconSource = LoadTrayIcon(),
            ToolTipText = "Screenshot Tray — click to see your recent screenshots",
